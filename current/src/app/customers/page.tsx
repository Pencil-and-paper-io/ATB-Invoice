...
export default function CustomersDirectoryPage() {
  ...
  const filteredSortedCustomers = useMemo(() => {
    ...
    const filtered = customers.filter((customer) => {
      if (archived.has(customer.id)) return false;
  ...
  }, [sortKey, sortDir, selectedTags, archivedIds]);
  ...
  const tableGridClass =
    "grid grid-cols-[1.4fr_1.2fr_0.95fr_0.85fr_0.95fr_1.1fr] gap-3";
      ...
      <main className="mx-auto max-w-6xl px-6 py-12 sm:px-8">
        ...
        <div className="overflow-x-auto rounded-xl border border-black/10 bg-white">
          <div className="min-w-[880px]">
            <div
              className={`${tableGridClass} border-b border-black/10 bg-black/[0.02] px-5 py-3 text-xs font-semibold uppercase tracking-wide text-black/45`}
            >
              <SortHeader
                label="Legal name"
                active={sortKey === "name"}
                  ...
                  return (
                    <li key={customer.id}>
                      <Link
                        ...
                        className={`${tableGridClass} px-5 py-3.5 text-sm transition hover:bg-prime-blue/5 ${
